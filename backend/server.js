...
// ========================================
// RELATÓRIO - CLIENTES ATIVOS E MOTIVOS DE PERDA
// ========================================

app.get('/api/reports/clients-and-loss-reasons', async (req, res) => {
  try {
    const {
      startDate,
      endDate
...
app.get('/api/reports/commercial-complete-summary', async (req, res) => {
  try {
    ...
    const accountsSet = new Set(
      ACCOUNTS_NAMES.map(normalizeName)
    );

    // ========================================
    // CLIENTES ATIVOS - OPEN
    // ========================================
    ...
    activeClientsRaw.forEach((lead) => {
      const normalizedResponsible =
        normalizeName(lead.responsibleName);

      if (sdrSet.has(normalizedResponsible)) {
        activeSummary.sdr.openLeads++;
        activeSummary.sdr.uniqueClientsSet.add(lead.clientName);
        return;
      }

      if (novosNegociosSet.has(normalizedResponsible)) {
        activeSummary.novosNegocios.openLeads++;
        activeSummary.novosNegocios.uniqueClientsSet.add(lead.clientName);
        return;
      }

      if (accountsSet.has(normalizedResponsible)) {
        activeSummary.accounts.openLeads++;
        activeSummary.accounts.uniqueClientsSet.add(lead.clientName);
        return;
      }

      activeSummary.outros.openLeads++;
      activeSummary.outros.uniqueClientsSet.add(lead.clientName);
    });

    const activeClients = {
      geral: activeSummary.geral,
    ...
    const totalLost = lossReasons.reduce(
      (sum, item) => sum + Number(item.total || 0),
      0
    );
    ...
    res.json({
      ...
      routeVersion:
        'clients-and-loss-reasons-v1',

      filters: {
        startDate: rangeStart,
        endDate: rangeEnd,
        activeClientsRule: 'status 0 / Open',
        lossReasonRule: 'status 11 / Lost com closedTime dentro do período'
      },
      ...
      lossReasons: lossReasons.map((item) => ({
        reason: item._id,
        total: Number(item.total || 0),
        percent:
          totalLost > 0
            ? (Number(item.total || 0) / totalLost) * 100
            : 0,
    ...
    console.error(
      'ERRO REPORT CLIENTS AND LOSS REASONS:',
      error
    );

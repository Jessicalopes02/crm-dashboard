...
function escapePerformanceRegex(value) {
...
function createPerformanceNameRegex(value) {
  const cleanName = String(value || '')
    .replace(/\s+/g, ' ')
    .trim();

  const escapedName =
    escapePerformanceRegex(cleanName).replace(
      /\s+/g,
      '\\s+'
    );
...
function getActivityName(activity) {
  return normalizePerformanceName(
    activity?.name ||
...
function getActivityOwnerName(activity) {
  return (
...
// ========================================
// PERFORMANCE DE EQUIPE - SDR / CLOSER
// ========================================
...
async function getTeamPerformanceDashboard({
  ...
  goals.forEach((goal) => {
    const name = String(goal.userName || '')
      .replace(/\s+/g, ' ')
      .trim();
    ...
    const normalized =
      normalizePerformanceName(name);

    if (!usersMap.has(normalized)) {
      usersMap.set(normalized, {
        name,
        goal
      });
    }
  ...
  const users = await Promise.all(
    configuredUsers.map(async ({
      name,
      goal
    }) => {
      const assigneeRegex =
        createPerformanceNameRegex(name);
      ...
      activityLeads.forEach((lead) => {
        ...
        leadActivities.forEach((activity) => {
          ...
          uniqueWorkedLeadIds.add(
            String(lead.nutshell_id)
          );
      ...
      const revenue = Number(
        revenueRaw[0]?.revenue || 0
      );

      const averageTicket = Number(
        revenueRaw[0]?.averageTicket || 0
      );

      const openPipelineValue = Number(
        pipelineRaw[0]?.openPipelineValue || 0
      );
  ...
  const totals = users.reduce(
    (acc, user) => {
      ...
      acc.workedLeads +=
        Number(user.workedLeads || 0);

      acc.activities +=
        Number(user.activities || 0);

      acc.meetingsScheduled +=
        Number(
          user.meetingsScheduled || 0
        );

      acc.meetingsCompleted +=
        Number(
          user.meetingsCompleted || 0
        );

      acc.noShow +=
        Number(user.noShow || 0);
      ...
      acc.cancelled +=
        Number(user.cancelled || 0);

      acc.revenue +=
        Number(user.revenue || 0);

      acc.openPipelineValue +=
        Number(
          user.openPipelineValue || 0
        );
  ...
  return {
    ...
    configuration: {
      ...
      warning:
        configuredUsers.length === 0
          ? `Nenhum usuário encontrado em goals para sector=${role} e period=${period}.`
          : null
    },
    ...
    users: users.sort((a, b) => {
      ...
      return (
        Number(b.workedLeads || 0) -
        Number(a.workedLeads || 0)
      );
    }),
    ...
    dataAvailability: {
      ...
      message:
        'Atividades e reuniões refletem somente as leads que já possuem activities sincronizadas no MongoDB.'
    }
  };
}
